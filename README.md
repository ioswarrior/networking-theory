...
Данные на сервере могут храниться в другом формате например в двоичном, чтобы занимать меньше места и для дальнейшей работы с ними их необходимо преобразовывать в универсальный JSON формат, который в дальнейшем мы уже расшифровываем и переводим в объекты нашей среды программирования. Данный процесс называется сериализацией. Сериализация - это процесс перевода данных из одного вида хранения в другой без потери содержания. Для перевода данных из одного формата в другой мы воспользуемся экземпляром класса JSONSerialization и обернем его в конструкцию do catch, так как в процессе преобразования данных всегда может пойти что-то не так. 
